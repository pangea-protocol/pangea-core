...
task("yieldPool:create", "create custom Pool: yieldPool")
  ...
  .setAction(
    async (
      ...
      const masterDeployer = (await ethers.getContract(
        "MasterDeployer"
      )) as MasterDeployer;
      ...
      const tokenContract = await IERC20Metadata__factory.connect(
        token,
        ethers.provider
      );
      const yieldTokenContract = await IERC20Metadata__factory.connect(
        yieldToken,
        ethers.provider
      );
      ...
      if (token.toLowerCase() < yieldToken.toLowerCase()) {
        ...
        price = calculatePrice(
          ...
          ethers.utils.parseUnits(
            yieldTokenAmount,
            await yieldTokenContract.decimals()
          )
        );
      } else {
...
task("miningPool:create", "create custom Pool: miningPool")
  ...
  .setAction(
    async (
      ...
      const token0 = await IERC20Metadata__factory.connect(
        token0Address,
        ethers.provider
      );
      const token1 = await IERC20Metadata__factory.connect(
        token1Address,
        ethers.provider
      );
