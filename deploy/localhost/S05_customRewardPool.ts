...
const deployFunction: DeployFunction = async function ({
  ...
  const airdropDistributor = await ethers.getContract<AirdropDistributorV2>(
    "AirdropDistributorV2"
  );
  ...
  const factory = await ethers.getContract<MiningPoolFactory>(
    "MiningPoolFactory"
  );
  ...
  if ((await factory.poolsCount(KDAI.address, KORC.address)).eq(0)) {
    const [tokens, data] = createDeployData(
      ...
      WKLAY.address,
      2000,
      parseUnits("200000", await KDAI.decimals()),
      parseUnits("10400000", await KORC.decimals())
    );
    await doTransaction(masterDeployer.deployPool(factory.address, data));
  }

  const poolHelper = await ethers.getContract<ConcentratedLiquidityPoolHelper>(
    "ConcentratedLiquidityPoolHelper"
  );
  const poolManager = await ethers.getContract<MiningPoolManager>(
    "MiningPoolManager"
  );
  ...
  if ((await factory.poolsCount(WKLAY.address, KORC.address)).eq(0)) {
    const [tokens, data] = createDeployData(
      ...
      KDAI.address,
      2000,
      parseUnits("200000", await WKLAY.decimals()),
      parseUnits("216600", await KORC.decimals())
    );
    await doTransaction(masterDeployer.deployPool(factory.address, data));
  }
  ...
  const mintAndApprove = async (
    ...
    if (tokenAddress == WKLAY.address) {
      ...
      await doTransaction(
        WKLAY.connect(user).approve(poolManager.address, amount)
      );
    } else {
      ...
      await doTransaction(
        token.connect(user).approve(poolManager.address, amount)
      );
    }
  };
  ...
  const addLiquidity = async (
    ...
    const pool = await ethers.getContractAt<MiningPool>(
      "MiningPool",
      poolAddress
    );
    ...
    // @ts-ignore
    const _lowerPrice = _price
      .mul(sqrt(JSBI.BigInt(lower * 10000)).toString())
      .div(10000);
    // @ts-ignore
    const _upperPrice = _price
      .mul(sqrt(JSBI.BigInt(upper * 10000)).toString())
      .div(10000);
    ...
    let _currentTicks = (await poolHelper.getTickState(pool.address)).map(
      (x) => x.index
    );
  ...
  const { user100, user101, user103, user104, user106, user107 } =
  ...
  const mintAndAirdrop = async (poolAddress: string, amount: BigNumberish) => {
    ...
    if (tokenAddress == WKLAY.address) {
      ...
      await doTransaction(
        airdropDistributor
          .connect(deployer)
          .depositKlay(pool.address, { value: amount })
      );
    } else {
      const token = (await ethers.getContractAt(
        "ERC20Test",
      ...
      await doTransaction(
        token.connect(deployer).mint(deployer.address, amount)
      );
      await doTransaction(
        token.connect(deployer).approve(airdropDistributor.address, amount)
      );
      await doTransaction(
        airdropDistributor
          .connect(deployer)
          .depositToken(pool.address, token.address, amount)
      );
    }
  };
  ...
  await ethers.provider.send("evm_setNextBlockTimestamp", [
    nextEpochStartTime.add(1).toNumber(),
  ]);
...
function priceRatioX96(amount1: BigNumber, amount0: BigNumber): BigNumber {
  return BigNumber.from(
    encodeSqrtRatioX96(amount1.toString(), amount0.toString()).toString()
  );
}
...
function createDeployData(
  ...
  return [
    [token0, token1],
    encodeData(token0, token1, rewardToken, fee, price, getTickSpacing(fee)),
  ];
}
...
function getNearestUpperValidTick(
  priceSqrtRatio: BigNumber,
  tickSpacing: number
) {
...
function getNearestLowerValidTick(
  priceSqrtRatio: BigNumber,
  tickSpacing: number
) {
  const tickAtPrice = getTickAtSqrtRatio(priceSqrtRatio);
