...
contract ConcentratedLiquidityPoolManager is
    ...
    ReentrancyGuardUpgradeable
{
    ...
    using TickIndex for IConcentratedLiquidityPool;
    ...
    function mintNative(
        ...
        IConcentratedLiquidityPool _pool = IConcentratedLiquidityPool(pool);
    ...
    function _mint(
...

        {
            ...
            (uint128 amount0Actual, uint128 amount1Actual) = DyDxMath.getAmountsForLiquidity(
                ...
                IConcentratedLiquidityPool(position.pool).price(),
                liquidityMinted,
                true
            );
    ...
    function burn(
        ...
        if (amount < position.liquidity) {
            (token0Amount, token1Amount) = IConcentratedLiquidityPool(position.pool).burn(position.lower, position.upper, amount);
    ...
    function _settleFees(uint256 positionId) internal {
    ...
    function _collect(
        ...
        Position storage position = positions[positionId];
        (token0amount, token1amount) = IConcentratedLiquidityPool(position.pool).collect(
            position.lower,
            position.upper,
            position.feeOwed0,
            position.feeOwed1
