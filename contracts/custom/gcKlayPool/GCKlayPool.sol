...
contract GCKlayPool is IGCKlayPool, IConcentratedLiquidityPoolStruct, IPoolFactoryCallee, LPRewardCallee, Initializable {
    ...
    modifier settleYield() {
        ...
        _;
        _updateShareAndBalanceForGCKlay(_zeroForYield);
    }

    function initialize(bytes memory _deployData, address _masterDeployer) external initializer {
        (address _token0, address _token1, uint24 _swapFee, uint24 _tickSpacing, bool _zeroForYield) = abi.decode(
            _deployData,
            (address, address, uint24, uint24, bool)
        );
    ...
    function swap(bytes memory data) external lock settleYield returns (uint256 amountOut) {
        ...
        if (cache.input > 0) {
            Tick storage lastTick = zeroForOne ? ticks[ticks[TickMath.MIN_TICK].nextTick] : ticks[ticks[TickMath.MAX_TICK].previousTick];
    ...
    function _updateShareAndBalanceForGCKlay(bool _zeroForYield) internal {
        address token;
        if (_zeroForYield) {
            token = token0;
        ...
        shareReserve = _share(token);
    }

    function _share(address token) internal view returns (uint256) {
        return IGCKlay(token).sharesOf(address(this));
    }
    ...
    function _getYieldBalance(bool _zeroForYield) internal view returns (uint128) {
        ...
        return uint128(
            FullMath.mulDiv(
                IGCKlay(token).totalSupply(),
                shareReserve,
                IGCKlay(token).totalShares()
            )
        );
    }
}
