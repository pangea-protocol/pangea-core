...
/// @notice Contract for deploying GCKlay Pair Liquidity Pool
contract GCKlayPoolFactory is OwnableUpgradeable, IConcentratedLiquidityPoolFactory {
    ...
    function deployPool(bytes memory _deployData) external returns (address pool) {
        ...
        _deployData = abi.encode(tokenA, tokenB, swapFee, tickSpacing, tokenA == gcklay);
    ...
    function setRewardToken(address pool, address token) external onlyManager {
        ...
        IGCKlayPool(pool).registerRewardToken(token);
