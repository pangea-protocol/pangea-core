...
describe("Yield Pool SCENARIO:SWAP", function () {
  ...
  before(async () => {
    ...
    const [tokenN0, tokenN1] =
      yToken.address.toLowerCase() < wklay.address.toLowerCase()
        ? [yToken.address, wklay.address]
        : [wklay.address, yToken.address];
    await poolFactory.setAvailableParameter(
      tokenN0,
      tokenN1,
      rewardToken.address,
    ...
    const nativePoolAddress = (
      await poolFactory.getPools(yToken.address, wklay.address, 0, 1)
    )[0];
    nativePool = await ethers.getContractAt<YieldPool>(
      "YieldPool",
      nativePoolAddress
    );
  ...
  async function traderBalance() {
    return {
      ...
      token1: await yToken.balanceOf(trader.address),
    };
  }
  ...
  describe("EXACT OUTPUT & EXACT OUTPUT SINGLE CASE", async () => {
    ...
    it("exactOutputSingle erc20 --> erc20", async () => {
      ...
      expect(await yToken.balanceOf(trader.address)).to.be.eq(amountOut);
    });
    ...
    it("TEST 1) EXACT OUTPUT SINGLE ERC20 --> ERC20 CASE> ", async () => {
      ...
      const realOutput =
        tokenIn.address === token0.address
          ? await yToken.balanceOf(trader.address)
