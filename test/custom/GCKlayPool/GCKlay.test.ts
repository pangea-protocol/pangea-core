...
describe("GCKlay Token TEST", function () {
  ...
  before(async () => {
    ...
    [user1, user2, user3, user4, user5, operator] = await ethers.getSigners();

    const MockToken = await ethers.getContractFactory("MockGCKlay");
    mockGCKlay = (await MockToken.deploy()) as MockGCKlay;
  ...
  async function depositReward(value: BigNumberish) {
    await operator.sendTransaction({ to: mockGCKlay.address, value });
  }
  ...
  describe("Transfer Test (Simple Case)", async () => {
    it("case 1) ", async () => {
       ...
       * [2] user2가 10 KLAY만큼 스테이킹
       * [3] 보상으로 1.1 KLAY 만큼 수취
       * 인 상황에서
       *
      ...
      await mockGCKlay.connect(user1).transfer(user2.address, transferAmount);
  ...
  it("case 2) ", async () => {
    ...
    await depositReward(ethers.utils.parseEther("2"));
  ...
  it("case 4) ", async () => {
    ...
    await mockGCKlay
      .connect(user2)
      .stake({ value: ethers.utils.parseEther("18") });
    await depositReward(ethers.utils.parseEther("10"));
  ...
  it("case 6) ", async () => {
     ...
     * [2] gcKlay re-staking이 0 ~ 10 KLAY 사이 랜덤 값으로 스테이킹
     ...
     */
    // GIVEN
    await mockGCKlay.connect(user1).stake({
      value: randomBN(ethers.utils.parseEther("1000")).add(
        ethers.utils.parseEther("1")
      ),
    });
    await depositReward(randomBN(ethers.utils.parseEther("10")));
    await mockGCKlay
      .connect(user2)
    ...
    for (let i = 0; i < 1000; i++) {
      ...
      await depositReward(increasedAmount);
