...
describe("GCKlay Pool SCENARIO:FEE", function () {
  ...
  async function lpBalance() {
    return {
      ...
      token1: await mockGCKlay.balanceOf(liquidityProvider.address),
  ...
  describe("# FEE DISTRIBUTION WITH YIELD CASE", async () => {
    ...
    it("TEST 1) increases total Staking = 10 KLAY", async () => {
      ...
      // Test yTokenEarned
      const yTokenEarned1 = (await poolManager.positionFees(lp.positionId))
        .token1amount;

      expect(
        givenTotalStaking.mul(9).div(10).sub(yTokenEarned1).abs()
      ).to.be.lte(DUST);
    });
    ...
    it("TEST 3) mint after increases total Staking = 1 KLAY", async () => {
      // FIRST, increaseTotal Staking
      await mockGCKlay
        .connect(liquidityProvider)
        .stake({ value: ethers.utils.parseEther("100") });
    ...
    it("TEST 5) partial burn and collect after increases total Staking = 10 KLAY", async () => {
      ...
      expect(
        beforeYTokenEarned
          .sub(await mockGCKlay.balanceOf(liquidityProvider.address))
          .abs()
      ).to.be.lte(DUST);
    });
  });
