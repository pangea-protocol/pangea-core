...
/**
 * Test for Mint & Burn in GCKlay Pool
 *
 ...
 *
 */
describe("GCKlay Pool SCENARIO:MINT_AND_BURN", function () {
  ...
  describe("# MULTIPLE MINT CASE", async () => {
    ...
    it("REVERT CASE : mint Overflow", async () => {
      ...
      await mockGCKlay.stake({ value: amountDesired.mul(2) });
      await mockGCKlay
        .connect(liquidityProvider)
        .approve(poolManager.address, amountDesired.mul(2));
    ...
    it("REVERT CASE : NOT OWNER", async () => {
      ...
      await mockGCKlay.stake({ value: amountDesired });
      await mockGCKlay
        .connect(trader)
        .approve(poolManager.address, amountDesired);
