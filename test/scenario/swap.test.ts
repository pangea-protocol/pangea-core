...
/**
 * Test for Swap in Concentrated Liquidity Pool
 *
 ...
 *
 */
describe("SCENARIO:SWAP", function () {
  ...
  describe("EXACT OUTPUT & EXACT OUTPUT SINGLE CASE", async () => {
    ...
    it("exactOutputSingle erc20 --> erc20", async () => {
      ...
      expect(await token1.balanceOf(trader.address)).to.be.eq(amountOut);
    });
  ...
  describe("# SWEEP CASE", async () => {
    it("token sweep", async () => {
      // GIVEN
      ...
      // THEN
      expect(balance.token1.eq(inputAmount)).to.be.true;
    });
    ...
    it("token sweep", async () => {
      ...
      // THEN
      expect(result).to.be.eq(inputAmount);
    });
  });
});
