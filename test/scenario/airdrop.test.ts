...
describe("SCENARIO:AIRDROP", function () {
  ...
  describe("# depositAirdrop", async () => {
    ...
    it("givenAirdrop0 & givenAirdrop1 & startTime & period all will be 0 if airdrop0, airdrop1 is 0 ", async () => {
      const givenAirdropStartTime =
        (await ethers.provider.getBlock("latest")).timestamp + 3600;
      const givenAirdropPeriod = WEEK;
      const givenAirdrop0 = ethers.utils.parseEther("10");
      ...
      // THEN
      expect(await pool.airdropStartTime()).to.be.eq(0);
      expect(await pool.airdropPeriod()).to.be.eq(0);
