...
describe("SCENARIO:EDGE CASE", function () {
  ...
  before(async () => {
    ...
    // ======== SIGNER ==========
    [deployer, liquidityProvider, trader, airdropManager] = await ethers.getSigners();
  ...
  async function burnAll(positionId: BigNumberish) {
    const result = await poolManager
        ...
        .callStatic
        .burn(positionId,
            (await poolManager.positions(positionId)).liquidity,
            liquidityProvider.address,
            0,
            0,
            false
  ...
  describe("# BURN ALL LIQUIDITY CASE ", async () => {
    ...
    it("TEST 1) swap direction : price down", async () => {
      ...
      const expectedOutput = (await getDy(lp1, targetPrice, currentPrice, true))
          ...
          .div(SWAP_BASE);
      const expectedSwapFee = (await getDy(lp1, targetPrice, currentPrice, true))
      ...
      expect(withInPrecision(feeResult1.token1amount, expectedSwapFee, 8)).to.be.true;
    ...
    it("TEST 2) swap direction : price up", async () => {
      ...
      const expectedOutput = (await getDx(lp3, currentPrice, targetPrice,  true))
          .mul(SWAP_BASE - SWAP_FEE)
          .div(SWAP_BASE);
      const expectedSwapFee = (await getDx(lp3, currentPrice, targetPrice, true))
          .mul(Math.floor(SWAP_FEE * 0.9))
          .div(SWAP_BASE)
      const poolPrice = await pool.price();
      ...
      expect(withInPrecision(feeResult3.token0amount, expectedSwapFee, 8)).to.be.true;
      expect(feeResult3.token1amount).to.be.eq(0);
    ...
    it("TEST 3) swap direction : price down and Up", async () => {
      ...
      {
        const span0Input = await getDy(lp1,
            await getPriceAtTick(-4 * TICK_SPACING),
            await getPriceAtTick(-3 * TICK_SPACING),
        true
        );
        const span1Input = await getDy(lp3,
            await getPriceAtTick(2 * TICK_SPACING),
            await getPriceAtTick(3 * TICK_SPACING),
            true
        );
        const span0Output = await getDx(lp1,
            await getPriceAtTick(-4 * TICK_SPACING),
            await getPriceAtTick(-3 * TICK_SPACING),
            true
        );
        const span1Output = await getDx(lp3,
            await getPriceAtTick(2 * TICK_SPACING),
            await getPriceAtTick(3 * TICK_SPACING),
            true
        );
        ...
        // TEST SWAP RESULT
        expect(withInPrecision(await getPriceAtTick(3 * TICK_SPACING), poolPrice, 8)).to.be.true;
        expect(withInPrecision(result, expectedOutput, 8)).to.be.true;

        const feeResult1 = await collectFees(1);
        const feeResult3 = await collectFees(3);
        expect(withInPrecision(feeResult1.token0amount, span0Output
            .mul(Math.floor(SWAP_FEE * 0.9))
            .div(SWAP_BASE), 8)).to.be.true;
        expect(withInPrecision(feeResult3.token0amount, span1Output
            .mul(Math.floor(SWAP_FEE * 0.9))
            .div(SWAP_BASE), 8)).to.be.true;
      }
    ...
    it("TEST 4) swap direction : price up and down", async () => {
      ...
      // SECOND, TRY TO SWAP
      {
        const spanInput = await token1Between(lp3, 2 * TICK_SPACING, 3 * TICK_SPACING);
        await clearBalance();
        await swapToken1ToToken0(spanInput);
      }

      {
        const {token0:span0Token0, token1:span0Token1} = await tokenBetween(lp3, 2 * TICK_SPACING, 3 * TICK_SPACING);
        const {token0:span1Token0, token1:span1Token1} = await tokenBetween(lp1, -4 * TICK_SPACING, -3 * TICK_SPACING);
        ...
        expect(withInPrecision(await getPriceAtTick(-4 * TICK_SPACING), poolPrice, 8)).to.be.true;
        expect(withInPrecision(result, expectedOutput, 8)).to.be.true;

        const feeResult1 = await collectFees(1);
        const feeResult3 = await collectFees(3);
        expect(withInPrecision(feeResult1.token1amount, span1Token1
            .mul(Math.floor(SWAP_FEE * 0.9))
            .div(SWAP_BASE), 8)).to.be.true;
        expect(withInPrecision(feeResult3.token1amount, span0Token1
            .mul(Math.floor(SWAP_FEE * 0.9))
            .div(SWAP_BASE), 8)).to.be.true;
      }
  ...
  describe("# BURN ALL LIQUIDITY CASE WITH AIRDROP", async () => {
    ...
    beforeEach("deploy PositionPool", async () => {
      ...
      period = 604_800;
      await token0.connect(airdropManager).mint(airdropManager.address, airdrop0.mul(10));
      await token1.connect(airdropManager).mint(airdropManager.address, airdrop1.mul(10));
      await token0.connect(airdropManager).approve(pool.address, airdrop0.mul(10));
      await token1.connect(airdropManager).approve(pool.address, airdrop1.mul(10));
    ...
    it("TEST 1) swap direction : price down", async () => {
      ...
      // SECOND, TRY TO SWAP
      const {token0: span0Token0, token1: span0Token1} = await tokenBetween(lp1, -3 * TICK_SPACING, -4 * TICK_SPACING);
      ...
      expect(withInPrecision(feeResult1.token1amount, expectedSwapFee.add(airdropFee1), 8)).to.be.true;
      expect(feeResult3.token0amount).to.be.eq(0);
      expect(feeResult3.token1amount).to.be.eq(0);
    ...
    it("TEST 2) swap direction : price up", async () => {
      ...
      expect(feeResult1.token0amount).to.be.eq(0);
      expect(feeResult1.token1amount).to.be.eq(0);
      expect(withInPrecision(feeResult3.token0amount, expectedSwapFee.add(airdropFee0),8)).to.be.true;
    ...
    it("TEST 3) re-distribution after airdrop (price down)", async () => {
      ...
      const feeResult3 = await collectFees(3);
      const feeResult1 = await collectFees(1);
      let airdropFee0 = airdrop0.mul(2).mul(timestamp - startTime).div(period)
      let airdropFee1 = airdrop1.mul(2).mul(timestamp - startTime).div(period)
    ...
    it("TEST 4) re-distribution after airdrop (price up)", async () => {
      ...
      // SECOND, TRY TO SWAP
      const {token0: span0Token0, token1: span0Token1} = await tokenBetween(lp1, -4 * TICK_SPACING, -3 * TICK_SPACING);
